...
import com.webninjas.wallpaper_advance.FullViewActivity;
...
public class ShowImagesAdapter extends PagerAdapter {
    ...
    public static void shareImage(Context context, String filePath) {
        try {
            ...
            intent.putExtra(Intent.EXTRA_TEXT, "share Image using");
            String path = MediaStore.Images.Media.insertImage(context.getContentResolver(), filePath, "", null);
            Uri screenshotUri = Uri.parse(path);
            intent.putExtra(Intent.EXTRA_STREAM, screenshotUri);
            intent.setType("image/*");
            context.startActivity(Intent.createChooser(intent, "No application found to open this file"));
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
...
//        Picasso.get().load(imageList.get(position).getPath()).into(imageView);
